...
RUN apt-get update && \
    ...
    xargs -a /deps/build-deps.txt apt-get install -y --no-install-recommends && \
...
### Cleaning up build dependencies"
    xargs -a /deps/build-deps.txt apt-get remove -y && \
...
### config directory and classpath directory
    mkdir -p /config/guacamole /var/lib/guacamole/ldap-schema /var/lib/guacamole/lib /var/lib/guacamole/extensions /etc/firstrun && \
### Tweak my.cnf
